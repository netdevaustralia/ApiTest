namespace Infrastructure.Services
{
    ...
    public class WooliesXService : IProductService
    {
        ...
        public WooliesXService(HttpClient httpClient,
            IOptions<ProductServiceConfig> productServiceConfig, ILogger<WooliesXService> logger)
        {
        ...
        public async Task<List<Product>> GetProductsAsync()
        {
            using var responseMessage =
                await _httpClient.GetAsync($"products?token={_productServiceConfig.Token}");
            try
            {
                var stream = await responseMessage.Content.ReadAsStreamAsync();
                responseMessage.EnsureSuccessStatusCode();
                var serializerOptions = JsonSerializerExtensions.GetDefaultJsonSerializerOptions();
                return await JsonSerializer.DeserializeAsync<List<Product>>(stream,
                    serializerOptions);
            }
            catch (Exception ex)
            {
                ...
                throw new ProductServiceException(
                    $"ProductServiceException.GetProductsAsync {ex.Message} {responseMessage.StatusCode}");
            }
        }
